...
## Simple V4: reliable scan instead of unstable live guesses

V4 keeps the fixed green rectangle, but the camera no longer trusts one frame. Put one piece fully inside the green box, hold it still, and press **SPACE** once. The scanner collects a short burst (12 predictions by default), checks how many frames agree, averages the probabilities, checks the top-1/top-2 margin, and only then returns a type. If the evidence is inconsistent, it says **NOT SURE - SCAN AGAIN** rather than forcing a wrong answer.

Run after training:

```powershell
.\.venv\Scripts\python.exe -m src.simple_camera --artifact artifacts/core_classifier_simple_v4 --config configs/default.yaml
```
...
Controls: `SPACE` scan, `S` save the current ROI, `R` clear the displayed result, `Q`/`Esc` quit.

Training V4 also enables class-balanced sampling and presents multiple independently augmented views of training originals per epoch. This affects only the training split; validation and test originals remain untouched.

## Phone camera testing (no retraining required)

This version can use a phone camera as the video source while keeping the same fixed green ROI and multi-frame voting logic.

### Option A — live phone stream over the same Wi-Fi

Use any phone camera application that exposes a standard **MJPEG/HTTP** or **RTSP** stream URL. The exact URL is shown by the phone application. Typical URL shapes are:
...
The phone and PC must be reachable on the same local network. Before loading the ML model, verify that OpenCV can read the phone stream:

```powershell
.\.venv\Scripts\python.exe -m src.check_video_source --source "http://PHONE_IP:PORT/video" --show
```

If the test window shows the phone image, start the real scanner:

```powershell
.\run_phone_camera.bat "http://PHONE_IP:PORT/video"
```

or directly:

```powershell
.\.venv\Scripts\python.exe -m src.simple_camera `
  --artifact artifacts/core_classifier_simple_v4 `
  --config configs/default.yaml `
  --source "http://PHONE_IP:PORT/video" `
  --no-mirror
```

The PC still displays the **green rectangle**. Point the phone at the piece, place the entire piece inside the green rectangle visible on the PC screen, hold the phone/piece steady, then press **SPACE** on the PC to run the multi-frame vote.

Controls remain:

```text
SPACE  scan the piece across multiple frames
S      save the current ROI crop
R      clear the current result
Q/ESC  quit
```

### Option B — phone already appears as a Windows webcam

If software such as a phone-webcam bridge exposes the phone as a normal Windows camera device, no URL is needed. Try camera indexes:

```powershell
.\.venv\Scripts\python.exe -m src.simple_camera --artifact artifacts/core_classifier_simple_v4 --source 0
.\.venv\Scripts\python.exe -m src.simple_camera --artifact artifacts/core_classifier_simple_v4 --source 1
.\.venv\Scripts\python.exe -m src.simple_camera --artifact artifacts/core_classifier_simple_v4 --source 2
```

### Important phone-camera accuracy note

Do not retrain only because the video source changed. First test the existing V4 artifact. However, if the phone camera produces noticeably different color, focal length, distance, or perspective from the camera used to build the dataset, collect some real training images **from that phone camera** and retrain. This domain match usually matters more than using a larger neural network.

Network streams are not mirrored by default because rear phone cameras normally provide the natural orientation. Use `--mirror` only if your stream is visibly reversed.
