...
def _weighted_sampler(
    dataset: CoreImageDataset, class_names: list[str], samples_per_epoch_multiplier: int = 1
) -> WeightedRandomSampler:
    """Balance classes and expose each training original to multiple augmentations per epoch.

    This changes only the TRAIN sampling process. Validation/test originals are never
    duplicated or augmented, so evaluation remains leakage-safe.
    """
    counts = dataset.class_counts
    ...
    multiplier = max(1, int(samples_per_epoch_multiplier))
    return WeightedRandomSampler(
        sample_weights, num_samples=len(sample_weights) * multiplier, replacement=True
    )
...
def train(config: dict[str, Any], evaluation_mode: str | None = None, fold: int | None = None) -> Path:
    ...
    if bool(training_cfg.get("weighted_sampler", False)):
        multiplier = int(training_cfg.get("samples_per_epoch_multiplier", 1))
        sampler = _weighted_sampler(train_dataset, class_names, multiplier)
        LOGGER.info(
            "WeightedRandomSampler enabled for training only (samples/epoch x%d).",
            max(1, multiplier),
        )
